...
public class Chunk : MonoBehaviour
{
    ...
    void GenerateBlockData()
    {
        ...
        mainNoise.addChannel(new Channel("Height", Algorithm.Simplex2d, 100.0f, NoiseStyle.Linear, 45f, 90f, Edge.Smooth).setFractal(4, 1.9f, 0.8f));
        ...
        for (int x = 3; x < chunkSize - 2; x++)
        {
            for (int z = 3; z < chunkSize - 2; z++)
            {
                if (Random.Range(0, 100) < 2)
                {
                    ...
                    PlantTree(new Vector3Int(x, topBlock, z), TreeType.Oak);
                }
            }
        }
    }
