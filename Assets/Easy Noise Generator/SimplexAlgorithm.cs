...
{
	int[][] grad3 = {new int[]{1,1,0},new int[]{-1,1,0},new int[]{1,-1,0},new int[]{-1,-1,0},
	...
	138,236,205,93,222,114,67,29,24,243,72,141,128,195,78,66,215,61,156,180};
	...
	public float noise2D(Vector3 v) 
	{
		double xin = v.x;
		double yin = v.z;
		...
		return 0.5f+0.5f*70.0f * (float)(n0 + n1 + n2);
	}
	
	public float noise3D(Vector3 v) 
	{		
		double xin = v.x;
		double yin = v.y;
		double zin = v.z;
		...
		// Add contributions from each corner to get the final noise value.
		// The result is scaled to stay inside [0...1]
		return 0.5f+32.0f*(float)(n0 + n1 + n2 + n3);
	}
}
